  ...
  router.put('/cartUpdate/:m_id/:sid', async (req, res) => {
    try {
      ...
      const promises = cartData.map((item) => {
        const sql = 'UPDATE cart_details SET quantity = ? WHERE sid = ? AND m_id = ?';
...
router.get('/member/:sid', async (req, res) => { 
  ...
  router.post('/order/:m_id', async (req, res) => {
    try {
      ...
      const orderSql = "INSERT INTO `product_order`(`m_id`, `orderId`, `amount`, `addressee`, `address`, `phone`, `created_at`, `payment`) VALUES (?,?,?,?,?,?,NOW(),?)";
      const [orderRows] = await db.query(orderSql, [req.params.m_id, orderId, amount, addressee, address, phone, payment]);
      const products_order_sid = orderRows.insertId;
      const detailsSql = "INSERT INTO `product_order_details`(`products_order_sid`, `product_id`, `price`, `quantity`) VALUES (?,?,?,?)";
...
router.get('/classOrder/:class_form_sid', async (req, res) => { 
  try {
    const sql = 'SELECT classform.*, classtime.*, class_order.*FROM classform INNER JOIN classtime ON classform.class_time = classtime.classtime INNER JOIN class_order ON class_order.class_form_sid = classform.sid WHERE class_order.class_form_sid =? '
...
router.get('/classForm/:class_form_sid', async (req, res) => { 
  const sql = 'SELECT * FROM classform INNER JOIN classbooking ON classform.sid=classbooking.classformsid WHERE classform.sid =?'
  const [rows] = await db.query(sql, [req.params.class_form_sid])
  res.json(rows)
})
...
router.post('/upDataClassForm/:class_form_sid', async (req, res) => {
  try {
    ...
    const orderSql = "INSERT INTO `class_order`(`orderId`, `m_id`, `class_form_sid`, `amount`, `created_at`) VALUES (?, ?, ?, ?, NOW())";
    const [orderRows] = await db.query(orderSql, [orderId, m_id, req.params.class_form_sid, amount]);
